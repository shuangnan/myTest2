...
<template>
	<el-container style="width:100%;height:100%;border-bottom-left-radius: 6px;" :style="{background:skinBlue?'rgba(67,90,121,0.6)':'aliceblue',color:skinBlue?'white':''}">
				...
				<div style="font-size: 12px;padding: 0px 20px 0px 0px;">
					...
					<div style="border-top-right-radius: 15px;border-bottom-right-radius: 15px;padding-left: 15px;padding-top: 3px;padding-bottom: 9px;"
					 :style="{background: skinBlue?'rgba(5,23,43,.4)':''}">
						...
						<div class="base-item"><label>运行区间</label>{{base.runStation}}</div>
								...
								<tr v-for="item in data_sc">
									...
									<td>
										<div class="per" v-for="per in item.children">
											<span>{{per.E_Name}}（{{per.KYD}}）</span><label class="tel">TEL:{{per.E_Phone}}</label>
										</div>
									</td>
								</tr>
							</tbody>
		...
		<el-footer style="padding: 5px;height: 40px;border-top: 1px dashed #EEE8D5;">
			暂无交路数据
		</el-footer>
	</el-container>
</template>
