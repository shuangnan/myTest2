 ...
 * @path:		http://10.192.6.155:9090/JFLCMS/vue/#/trainTravelInfo?id=118672757&blue=true
 -->
<template>
	<el-container style="width:100%;height:100%;border-bottom-left-radius: 6px;" :style="{background:skinBlue?'rgba(67,90,121,0.6)':'aliceblue',color:skinBlue?'white':''}">
			...
			<el-aside style="width:320px;height:100%;">
				<div style="font-size: 12px;padding: 0px 20px 0px 0px;">
					<div class="base-station" :style="{color: skinBlue?'#fafafa':''}" alt="发到站">
						...
						<span class="base-zwd" :style="{color: base.lateTime==='正点'?'#92E252':'#F1A1A0'}">{{base.lateTime}}</span>
					</div>

					<div style="border-top-right-radius: 15px;border-bottom-right-radius: 15px;padding-left: 15px;padding-top: 3px;padding-bottom: 9px;"
					 :style="{background: skinBlue?'rgba(5,23,43,.4)':''}">
						<div class="base-item" style="padding-top: 8px;"><label>始发日期</label>{{base.startDate}}</div>
						<div class="base-item"><label>运行状态</label>{{base.curStatuStr}}</div>
						<div class="base-item"><label>运行区间(站)</label>{{base.runStation}}</div>
						<div class="base-item"><label>{{base.timeTitle1}}</label>{{base.timeValue1}}</div>
						<div v-if="base.timeVisible2" class="base-item"><label>{{base.timeTitle2}}</label>{{base.timeValue2}}</div>
					</div>

					<!-- <div class="base-item" style="padding-left: 15px;padding-top: 6px;">当前通勤人数：<span :style="{color: skinBlue?'white':'#7fadcf'}">{{curNumTQ}}人</span></div> -->
					<div style="float: right;padding-bottom: 5px;padding-left: 15px;padding-top: 6px;">
						<!-- <span style="display: inline-block;vertical-align: middle;padding-right: 10px;">安全员</span> -->
					...
					<div style="padding-left: 15px;padding-top: 2px;">
						<div style="padding: 5px 0;">
							<span style="display: inline-block;vertical-align: middle;">安全员</span>
							<table class="list-per" :class="{'blue':skinBlue}" cellpadding="0" cellspacing="0">
								<tbody>
									<tr v-for="per in data_aqy">
										<td>
											<span>{{per.E_Name}}（{{per.KYD}}）</span><label class="tel">{{per.isGW?'G网':'TEL'}}:{{per.E_Phone}}</label>
										</td>
									</tr>
								</tbody>
							</table>
						</div>
						<span style="display: inline-block;vertical-align: middle;">三乘人员</span>
						<table class="list-per" :class="{'blue':skinBlue}" cellpadding="0" cellspacing="0">
							<tbody>
			...
			<el-main style="padding:0 13px 0px 0px;overflow: hidden;">
				<el-tabs type="card" size="mini" v-model="curTab" class="my-tab" :class="{'blue':skinBlue}">
					<el-tab-pane label="乘客" name="ck" style="height: 100%;">
						<el-table :data="data_ck" size="mini" :stripe="true" border :highlight-current-row="false" height="100%">
							<el-table-column label="车站" prop="cz" align="center" min-width="50" :show-overflow-tooltip="true"></el-table-column>
					...
					<el-tab-pane label="运行状况" name="yxzk" style="height: 100%;">
						<el-table :data="data_yxzk" size="mini" :stripe="true" border :highlight-current-row="false" :row-class-name="highlightRow_yxzk"
						 height="100%">
							<el-table-column label="序" prop="cxh" width="20" align="center"></el-table-column>
							<el-table-column label="站名" prop="czm" align="center" min-width="50" :show-overflow-tooltip="true"></el-table-column>
					...
					<el-tab-pane label="时刻表" name="skb" style="height: 100%;">
						<el-table :data="data_skb" size="mini" :stripe="true" border :highlight-current-row="false" height="100%">
					...
					<!-- <el-tab-pane label="交路" name="tq" style="height: 100%;">
						<el-table :data="data_jl" size="mini" :stripe="true" border :highlight-current-row="false" height="100%">
					...
					<el-tab-pane label="编组" name="bz" style="height: 100%;">
						<el-table :data="data_bz" size="mini" :stripe="true" border :highlight-current-row="false" height="100%">
							...
							<template v-else>
								<el-table-column label="饮水设备" prop="bz_yssb" align="center" min-width="50">
									<template slot="header">饮水<br />设备</template>
								</el-table-column>
								...
								<el-table-column label="座椅形式" prop="bz_zyxs" align="center" min-width="50">
									<template slot="header">座椅<br />形式</template>
								</el-table-column>
							</template>
						</el-table>
					</el-tab-pane>
		...
		<el-footer style="padding: 5px;height: 100px;border-top: 1px dashed #EEE8D5;">
			<train-jl-line :ccId="trainId || $store.state.params.id" :cc="trainInfo.trainNum"></train-jl-line>
		</el-footer>
	</el-container>
</template>
...
<script>
	...
	import TrainJlLine from "./TrainJlLine1.vue"
	const graphUrl = "http://10.192.34.79/GraphH5/services/GeneralTicketStreamServlet"; //在途列车接口地址
		...
		methods: {
			...
			getDatas() {
				...
				this.getTrainInfoByTrainid(id, () => {
					...
					if (zt === "0") { //在站
						this.base.timeTitle1 = "到达时间"; //时间标题1
						this.base.timeValue1 = this.urlP.dqzsj_in; //时间值1
						this.base.timeTitle2 = "预计出发时间"; //时间标题2
			...
			openWin(per) {
				...
				const webP = this.$store.state.params;
				const pos = "&UserName=" + webP.UserName + "&UserPID=" + webP.UserPID + "&Oid=" + webP.Oid + "&Oname=" + webP.Oname;
				...
				C.getData({
					...
					if (r && r.status) {
						...
						const trainInfo = {
							...
							sfrq: r.msg[1].substring(0, 4) + "-" + r.msg[1].substring(4, 6) + "-" + r.msg[1].substring(6), //车次日期
						}
				...
				C.getData({
					...
					if (ck && ck.length > 0) {
						...
						for (let i = 0; i < ck.length; i++) {
							var d = ck[i];
							let sc = parseInt(d[12]) + parseInt(d[13]) + parseInt(d[4]) + parseInt(d[5]) + parseInt(d[6]) +
								parseInt(d[7]) + parseInt(d[8]) + parseInt(d[9]) + parseInt(d[10]) + parseInt(d[11]);
							if (isNaN(sc)) {
								sc = 0;
							}
							let xc = parseInt(d[22]) + parseInt(d[23]) + parseInt(d[14]) + parseInt(d[15]) + parseInt(d[16]) +
								parseInt(d[17]) + parseInt(d[18]) + parseInt(d[19]) + parseInt(d[20]) + parseInt(ck[i][21]);
							if (isNaN(xc)) {
								xc = 0;
							}
							sum += sc;
				...
				C.getData({
					...
					if (sk && sk.length > 0) {
						...
						for (var i = 0; i < sk.length; i++) {
							...
							if (obj.czm == this.urlP.dqz || obj.csjdf === "到达" && sk[i + 1] && sk[i + 1][10] < 1) {
								obj.curStn = true; //当前站
							}
							// 到达晚点情况
							if (sk[i][13] == 0) {
				...
				C.getData({
					...
					if (sk && sk.length > 0) {
						...
						for (var i = 0; i < sk[0].length; i++) {
							...
							if (sk[0][i].length === 2 && cxdyIndex === -1 && sk[0][i][0] !== "" && sk[0][i][1] !== "") {
								cxdyIndex = i;
							}
							data_bz.push(obj);
						}
			...
			getData_sc(ty) {
				...
				if (ty === null || ty === 0) {
					...
					now = C.getDateTimeSub(now, "yyyy-MM-dd", -24);
				} else if (ty === -2) {
					now = C.getDateTimeSub(now, "yyyy-MM-dd", -48);
				}
				C.getData({
					url: C.kydUrl,
					...
					if (r !== "null" && r.length > 0) {
						// 第一次取数据
						if (ty === null) {
							this.days_sx = Math.abs((Date.parse(new Date(r[0].ZD_date)) / 1000 - Date.parse(new Date(r[0].SF_date)) / 1000) /
								(3600 * 24));
						}
						var real = [];
						var realJson = {};
						...
						C.$.each(r, function(i, v) {
							v.isGW = v.E_Phone.indexOf("149") === 0;
							if (v.ZhiWei === "安全员") {
								safePer.push(v);
							} else if (v.ZhiWei === "司机") {

							} else if (typeof(realJson[v.ZhiWei]) === "undefined") {
								real.push({
									ZhiWei: v.ZhiWei,
									children: [v]
								});
						...
						this.data_aqy = safePer;
						this.data_sc = real;
						this.getData_sc_sj(now); //获取三乘信息(司机)
					} else {
						this.getData_sc_sj(now); //获取三乘信息(司机)
					}
				});
			...
			getData_sc_sj(now) {
				C.getData({
					...
					s: [{
						"cc": this.urlP.ccAll,
						"run_date": now.replace(/-/g, ""),
					}],
					a: "【eqeh_zxy2065】获取三乘信息(司机)"
				}, r => {
					if (r !== "null" && r.data["1"].length > 0) {
						const row = {
							ZhiWei: "司机",
							children: []
						}
						r.data["1"].forEach(v => {
							if (v.POST === "司机") {
								row.children.push({
									E_Name: v.NAME,
									KYD: "",
									E_Phone: v.GSMR ? v.GSMR : v.PHONE,
									isGW: v.GSMR || v.PHONE.indexOf("149") === 0
								});
							}
						});
						if (row.children.length > 0) {
							this.data_sc.push(row);
						}
...
<style scoped="scoped">
	...
	>>>.my-tab.el-tabs--top.el-tabs--card>.el-tabs__header .el-tabs__item:nth-child(1) {
		border-top-left-radius: 10px;
	}
	...
	>>>.blue .el-table tr.el-table__row {
		background-color: rgb(60, 81, 108, .6);
	}
	...
	>>>.blue .el-table--striped .el-table__body tr.el-table__row--striped.cur-stn td {
		background: #208B78;
		color: white;
	}
