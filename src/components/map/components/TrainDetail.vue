<!-- 列车详情 -->
<template>
	<div v-loading="loading" element-loading-text="正在获取数据..." element-loading-spinner="el-icon-loading"
	 element-loading-background="rgba(0, 0, 0, 0.7)" style="width:100%;height: 100%;padding: 5px;overflow: hidden;">
				...
				<tr>
							...
							<div class="train-info" style="padding: 10px 0;line-height: 20px;border-bottom: 2px solid #1c243b;">
								...
								<div>
									...
									<label v-if="b.STEWARD_NAME">
										...
										<el-image :src="msgPic" @click="toolClick('openMsg','self')" title="发送消息" class="img-cls"></el-image>
										<el-image :src="vidioPic" @click="toolClick('openXiaoYu','self')" title="音视频对讲" class="img-cls"></el-image>
										<el-image :src="phonePic" @click="toolClick('openPhone','self')" title="应急呼叫" class="img-cls"></el-image>
									</label>
									<label v-else-if="data_lcz.length===1">
										{{data_lcz[0].E_Name}} <a class="tel" @click="toolClick('openPhone',data_lcz[0])">{{data_lcz[0].E_Phone}}</a>
										<el-image :src="msgPic" @click="toolClick('openMsg',data_lcz[0])" title="发送消息" class="img-cls"></el-image>
										<el-image :src="vidioPic" @click="toolClick('openXiaoYu',data_lcz[0])" title="音视频对讲" class="img-cls"></el-image>
										<el-image :src="phonePic" @click="toolClick('openPhone',data_lcz[0])" title="应急呼叫" class="img-cls"></el-image>
									</label>
									<el-popover v-else-if="data_lcz.length>0" placement="right" width="300" trigger="click">
										<ul>
											<li v-for="lcz in data_lcz">
												...
												<el-image :src="msgPic" @click="toolClick('openMsg',lcz)" title="发送消息" class="img-cls"></el-image>
												<el-image :src="vidioPic" @click="toolClick('openXiaoYu',lcz)" title="音视频对讲" class="img-cls"></el-image>
												<el-image :src="phonePic" @click="toolClick('openPhone',lcz)" title="应急呼叫" class="img-cls"></el-image>
											</li>
										</ul>
					...
					<td valign="top">
						<div class="box" style="width: 840px;">
							...
							<div class="box-body" v-show="curTop!=='bz'" style="height: 350px;">
								<el-table ref="klfx" style="width:100%;" height="350px" size="mini" :data="klfxData" :border="false"
								 :show-summary="true" :summary-method="getSummary" :highlight-current-row="false">
		...
		<el-dialog :title="titleOfCS" :visible.sync="dialogVisible" width="600px" :append-to-body="false" :modal="false">
			<el-table v-loading="loading" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)"
			 style="width:100%;" height="400" size="mini" :data="detailCS" :border="false" :highlight-current-row="false">
...
<script>
	...
	const kydUrl = C.ADDS.servGS; //接口地址
	...
	export default {
		...
		data() {
			return {
				...
				g1: [], //餐售-渠道
				g1: [], //餐售-类型
				trainImg: require('../img/wayLog.png'),
		...
		methods: {
			init(datasource) {
				this.a = datasource["1"][0] || {} //始发终到
				this.b = datasource["2"][0] || {} //列车长
				const dataC = datasource["3"] //任务列表
				dataC.forEach(v => {
				...
				this.c = dataC;
				this.d = datasource["4"] //保洁打分
				this.e = datasource["5"] //备品签收情况
				this.f = datasource["6"] //客流分析
				this.g1 = datasource["7"] //餐售-渠道
				this.g2 = datasource["8"] //餐售-类型

				this.$nextTick(() => {
					if (!this.b.STEWARD_NAME) {
						this.getData_lcz(); //原数据无列车长，则从三乘中获取
					}
					this.getBz(); //编组
			...
			getClxq(cb) {
				...
				const _date = echarts.format.formatTime('yyyyMMdd', new Date(this.train.START_DATE).getTime());
				const p = [this.train.TRAIN, _date]
				// const p = ["G350", _date]
				this.detailSql = p;
				C.getDataP123({
					c: "kydp_zxy9002",
					s: [p],
					a: "【kydp_zxy9002】获取车辆详情",
					url: kydUrl
				}, r => {
					if (r) {
						cb(r);
...
<style scoped="scoped">
	...
	.timeline>li>.point>span {
		...
		height: 20px;
		border-radius: 50%;
		background: url(../img/trainDetail/trainInfoCircle.png)no-repeat;
		word-wrap: normal;
		white-space: nowrap;
	}
	...
	.timeline>li>.info {
		...
		padding-top: 7px;
		padding-left: 24px;
		background: url(../img/trainDetail/未开始.png) no-repeat;
		background-size: 100% 100%;
	}

	/* statu-a-b a(1-已完成，0-未完成) b(0-未超时，1-超时20~60 2-超时60以上)*/
	.timeline>li>.info.statu-0-0 {
		background: url(../img/trainDetail/未完成.png) no-repeat;
		background-size: 100% 100%;
	}

	/* 已完成- */
	.timeline>li>.info.statu-1-0 {
		background: url(../img/trainDetail/已完成.png) no-repeat;
		background-size: 100% 100%;
	}

	/* 已完成-已超时20~60 */
	.timeline>li>.info.statu-1-1 {
		background: url(../img/trainDetail/已超时020min.png) no-repeat;
		background-size: 100% 100%;
	}

	/* 未完成-已超时60以上 */
	.timeline>li>.info.statu-1-2 {
		background: url(../img/trainDetail/已超时60min.png) no-repeat;
		background-size: 100% 100%;
	}
