<template>
  <div class="effect-range-container" @mouseover="stopAutoScroll" @mouseout="enableAutoScroll">
    <div id="graph1" :style="{
      width: clientWidth + 'px',
      height: clientHeight + 'px'
    }">
    </div>
    <div id="trainTableToolTip" :style="{
      ...
      <div style="height: 100%; width: 100%; display: flex; flex-direction: column">
        <div style="height: 20px; width: 100%; display: flex; justify-content: space-between; cursor: pointer" @click="showTrainDetail = false">
          <span style="font-weight: bold; font-size: 18px; margin-left: 10px">车次详情</span>
          <span style="font-weight: bold; margin-right: 10px; font-size: 20px">x</span>
        </div>
        <div style="height: calc(100% - 20px); width: 100%; margin-top: 5px; display: flex; align-items: center; align-content: center; justify-content: center" class="train-detail-table">
          <el-table size="mini" :data="trainDetails" max-height="200px" style="display: flex; flex-direction: column; align-items: center; align-content: center; justify-content: center; margin-left: 30px">
    ...
    <div id="lineTrainDetailTable" :style="{
      ...
      <div style="height: 100%; width: 100%; display: flex; flex-direction: column">
        <div style="height: 20px; width: 100%; display: flex; justify-content: space-between; cursor: pointer"
             @click="showLineTrainDetail = false">
          <span style="font-weight: bold; font-size: 18px; margin-left: 10px">轨道车信息</span>
          <span style="font-weight: bold; margin-right: 10px; font-size: 20px">x</span>
        </div>
        <div
            style="height: auto; width: 100%; margin-top: 5px; display: flex; flex-direction: column; align-content: center; align-items: center">
          <div style="height: auto; width: 100%">
            <h4 style="color: white; margin-left: 10px">
              供电轨道车信息
            </h4>
            <div class="train-detail-table">
              <el-table :data="gdLineTrain" size="mini" max-height="200px">
                <el-table-column prop="DUAN_NAME" label="单位" align="center"></el-table-column>
                <el-table-column prop="LINE_NAME" label="线别" align="center"></el-table-column>
                <el-table-column prop="WORKTRAIN_NAME" label="车号" align="center"></el-table-column>
                <el-table-column prop="WORKTRAIN_TYPE" label="机车型号" align="center"></el-table-column>
                <el-table-column prop="PARK_PLACE" label="停驻站点" align="center"></el-table-column>
                <el-table-column prop="SJ_NAME" label="司机" align="center"></el-table-column>
                <el-table-column prop="SJ_PHONE" label="司机电话" align="center" width="105px"></el-table-column>
                <el-table-column prop="GL_NAME" label="管理人员" align="center"></el-table-column>
                <el-table-column prop="GL_PHONE" label="管理人员电话" align="center"></el-table-column>
                <el-table-column prop="WORK_RANGE" label="当前停留点" align="center"></el-table-column>
                <el-table-column prop="TRAIN_STATES" label="当前状态" align="center">
                  <template slot-scope="state">
                                        <span>{{
                                            state.row.TRAIN_STATES === '1' ? '可用' : (state.row.TRAIN_STATES === '2' ? '不可用' : '')
                                          }}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="REASON" label="不可用原因" align="center"></el-table-column>
                <el-table-column prop="INPUT_DATE" label="更新时间" align="center"></el-table-column>
              </el-table>
            </div>
          </div>
          <div style="height: auto; width: 100%">
            ...
            <div class="train-detail-table">
              <el-table :data="gwLineTrain" size="mini" max-height="200px">
                <el-table-column prop="DW" label="单位" align="center"></el-table-column>
                <el-table-column prop="LINE" label="线别" align="center"></el-table-column>
                <el-table-column prop="TRAIN_NO" label="车号" align="center"></el-table-column>
                <el-table-column prop="TRAIN_TYPE" label="机车型号" align="center"></el-table-column>
                <el-table-column prop="PLACE" label="停驻站点" align="center"></el-table-column>
                <el-table-column prop="PERSON_NAME" label="司机" align="center"></el-table-column>
                <el-table-column prop="PERSON_PHONE" label="司机电话" align="center" width="105px"></el-table-column>
                <el-table-column prop="CJ_NAME" label="管理人员" align="center"></el-table-column>
                <el-table-column prop="CJ_PHONE" label="管理人员电话" align="center"></el-table-column>
                <el-table-column prop="STATION" label="当前停留点" align="center"></el-table-column>
                <el-table-column prop="TRAIN_STATES" label="当前状态" align="center">
                  <template slot-scope="state">
                                        <span>{{
                                            state.row.TRAIN_STATES === '1' ? '可用' : (state.row.TRAIN_STATES === '2' ? '不可用' : '')
    ...
    <!--    <z-dialog title="查看影响图示" :show.sync="showDetail">-->
    <!--      <div slot="body">-->
    <!--        <EffectRangePreview-->
    <!--            :auto-fold="false"-->
    ...
    <!--            :stations="stations"-->
    <!--            :sections="sections"-->
    <!--            :range-start-mile="rangeStartMile"-->
    <!--            :range-end-mile="rangeEndMile"-->
    <!--        />-->
    <!--      </div>-->
    <!--    </z-dialog>-->
  </div>
...
export default {
  ...
  computed: {
    graphScale() {
      if (Math.abs(parseFloat(this.rangeEndMile) - parseFloat(this.rangeStartMile)) !== 0) {
        return (this.clientWidth / (Math.abs(parseFloat(this.rangeEndMile) - parseFloat(this.rangeStartMile))))
      } else {
        return 0
      }
    },
  ...
  methods: {
    ...
    kmToPx(km) {
      return (this.graphScale * (parseFloat(km) - parseFloat(this.rangeStartMile)))
    },
    /**
     * 绘制Stage
     */
    renderStage() {
      console.log('详情公里标', this.rangeStartMile, this.rangeEndMile, this.sections, this.clientWidth)
      if (this.layers.stage) {
        // 已存在Stage，清除内容
        this.layers.stage.destroy()
        console.log('Stage已销毁')
      }
      console.log('初始化Stage', this.$store.state.earthquake.currentRenderingSection.L_NAME)
      // 初始化Stage
      let stage = new Konva.Stage({
        container: '#graph1',
        x: 0,
        y: 0,
        width: this.clientWidth + 150,
      ...
      //   this.currentScale = this.currentScale * scaleBy
      //   this.renderStage()
      //
      ...
      this.renderRailLines().then(() => {
        this.renderStations().then(() => {
          this.renderTrains().then(() => {
      ...
      return new Promise((resolve, reject) => {
        try {
          ...
          this.sections.map((section) => {
            if (section.TYPE === '1') {
              // 封锁区间
              console.log(section.TYPE, this.kmToPx(section.S_MILE), this.kmToPx(section.E_MILE))
              let fsLineRect = new Konva.Rect({
                x: this.kmToPx(section.S_MILE) + 20,
                y: 0.5 * this.clientHeight - 10,
                width: this.kmToPx(section.E_MILE) - this.kmToPx(section.S_MILE),
                height: 4,
        ...
        try {
          ...
          this.stations.map((station) => {
            ...
            let railTrainIcon = new Konva.Image({
              ...
              y: 0.5 * this.clientHeight - 10 - 8.5 - 5 - 11.5,
              height: 11.5,
              width: 16,
              image: (station.GDRAILTRAIN.length + station.GWRAILTRAIN.length !== 0) ? this.trainIcons.lineTrain : null,
              name: 'railTrain'
            })
            ...
            let railTrainNum = new Konva.Text({
              ...
              y: 0.5 * this.clientHeight - 10 - 8.5 - 5 - 11.5,
              text: (station.GDRAILTRAIN.length + station.GWRAILTRAIN.length !== 0) ? (station.GDRAILTRAIN.length + station.GWRAILTRAIN.length) : '',
              fill: 'white',
              name: 'railTrainNum'
            })
            stationGroup.add(railTrainNum)
            console.log(this.kmToPx(station.CENTER_MILE))

            // 渲染车站图标
            let stationMarker = new Konva.Image({
              x: this.kmToPx(station.CENTER_MILE),
              y: 0.5 * this.clientHeight - 10 - 8.5,
              height: 17,
              width: 17,
            ...
            let stationNameString = new Konva.Text({
              x: this.kmToPx(station.CENTER_MILE),
              y: 0.5 * this.clientHeight - 10 + 8.5 + 5 - 1,
            ...
            let stationMileString = new Konva.Text({
              x: this.kmToPx(station.CENTER_MILE),
              y: 0.5 * this.clientHeight - 10 + 8.5 + 3 + 14,
            ...
            stationGroup.getChildren().map((ele) => {
              ...
              if (ele.attrs.name === 'stationName') {
                ...
                ele.x(ele.attrs.x + groupWidth * 0.5 - textWidth * 0.5)
              }
            ...
            stationGroup.on('click', function (e) {
              console.log('显示轨道车')
              let railTrainTableTooltip = document.getElementById('lineTrainDetailTable')
              _this.gwLineTrain = station.GWRAILTRAIN
              _this.gdLineTrain = station.GDRAILTRAIN
              _this.showLineTrainDetail = (station.GWRAILTRAIN.length + station.GDRAILTRAIN.length !== 0)
              _this.$nextTick(() => {
                railTrainTableTooltip.style.top = (e.evt.layerY - railTrainTableTooltip.clientHeight + 50) + 'px'
              })
            })
          ...
          stationsGroup.getChildren().map((group, index, arr) => {
            ...
            let overlayStations = arr.filter((station) => {
              ...
              if (groupX <= stationX) {
                ...
                // 被测元素在本站前
                if (groupX - stationX <= stationWidth) {
                  return true
                } else {
                  return false
                }
            ...
            console.log('详情重叠车站', overlayStations)
            if (overlayStations.length > 1) {
              overlayStations.map((ele, index) => {
                // ele.visible(false)
                // ele.x(ele.x() + 30)
                ele.offsetX(-index * 100)
              })
              ...
              //   visible: false
              // })
              // let overlayString = overlayStations.map((station) => {
              //   return station.getChildren().find((node) => {
              //     return node.attrs.name === 'stationName'
              //   }).attrs.text
              // }).join('/')
              // let stringLength = overlayString.length * 12 - 6 * (overlayStations.length - 1)
              // let stationStr = new Konva.Text({
              //   x: group.getChildren().find((node) => {
              //     return node.attrs.name === 'stationMarker'
              //   }).attrs.x - (0.5 * stringLength),
              //   y: 0.5 * this.clientHeight - 2 - 8.5 - 15,
              //   text: overlayString,
              //   fill: 'white'
              // })
              // overlayStationsPreview.add(stationStr)
              // overlayStationsGroup.add(overlayStationsPreview)
              // overlayStationsGroup.on('mouseover', function () {
      ...
      return new Promise((resolve, reject) => {
        try {
          ...
          this.trains.map((train) => {
            let direction = parseFloat(train.O_KM) <= parseFloat(train.E_KM) ? '上' : '下'
            let trainSectionStart = parseFloat(train.O_KM)
            let trainSectionEnd = parseFloat(train.E_KM)
            let trainPlacement = (trainSectionStart + trainSectionEnd) / 2
            ...
            let trainMarker = new Konva.Image({
              ...
              y: direction === '上' ? (0.5 * this.clientHeight - 10 - 12) : (0.5 * this.clientHeight - 10 + 10),
              width: 24.5,
              height: 10,
              image: direction === '上' ? this.trainIcons.upTrain : this.trainIcons.downTrain,
              name: 'trainMarker'
            })
            ...
            let trainNo = new Konva.Text({
              x: this.kmToPx(trainPlacement),
              y: direction === '上' ? (0.5 * this.clientHeight - 10 - 12 - 12) : (0.5 * this.clientHeight - 10 + 10 + 12),
              text: train.TRAIN,
              fill: 'white',
              name: 'trainNo'
            })
            ...
            trainGroup.getChildren().map((ele) => {
              ...
              if (ele.attrs.name === 'trainNo') {
                ...
                ele.x(ele.attrs.x + groupWidth * 0.5 - stringLength * 0.5)
              }
            })
          ...
          trainsGroup.getChildren().map((trainGroup, index, arr) => {
            ...
            if (overlayTrains.length > 1) {
              ...
              // 去折叠车次Marker图片
              let trainMarker = overlayTrains[0].getChildren().find((node) => {
              ...
              let overlayTrainCount = new Konva.Text({
                ...
                y: (trainMarker.y() > (0.5 * this.clientHeight - 10)) ? (trainMarker.y() + 15) : (trainMarker.y() - 15),
              ...
              overlayTrainGroup.on('click', function (e) {
                ...
                let trainTableToolip = document.getElementById('trainTableToolTip')
                ...
                trainTableToolip.style.top = (e.evt.layerY - 230) + 'px'
    ...
    renderEnvSection() {
      return new Promise((resolve, reject) => {
        ...
        let allSections = this.sections.sort((sectionA, sectionB) => {
          return parseFloat(sectionA.S_MILE) - parseFloat(sectionB.S_MILE)
        })
        let sectionList = []

        // 合并相同高层区间
        allSections.map((section, index, arr) => {
          console.log(section.S_MILE, section.E_MILE, section.merged)
          if ((section.merged === undefined || !section.merged)) {
            ...
            let type = (section.ENVIRONMENT === '平原' || section.ENVIRONMENT === '微丘') ? '1' : '2'
            ...
            for (let i = 0; i < behindSections.length; i++) {
              ...
              let currentStart = parseFloat(currentSection.S_MILE)
              let currentEnd = parseFloat(currentSection.E_MILE)
              let currentType = (currentSection.ENVIRONMENT === '平原' || currentSection.ENVIRONMENT === '微丘') ? '1' : '2'
        ...
        sectionList.map((section) => {
          ...
          let envLine = new Konva.Rect({
            ...
            height: 3,
            width: this.kmToPx(section.end) - this.kmToPx(section.start),
            fill: section.env === '1' ? '#B8E897' : '#FFA339'
          })

          let envIcon = new Konva.Image({
            x: (this.kmToPx(section.start) + this.kmToPx(section.end)) / 2 - 10.5,
            y: this.clientHeight - 5 - 16,
            width: 21,
            height: 12,
            image: section.env === '1' ? this.envIcons.pingyuan : this.envIcons.zhongqiu
          })
...
.train-detail-table >>> .el-table th, .el-table tr, cell {
...
.train-detail-table >>> .el-table--enable-row-hover .el-table__body tr:hover > td {
...
.train-detail-table >>> .el-table, .el-table__expanded-cell {
  background: transparent !important;
  border: 0 !important;
}

.train-detail-table >>> .el-table--border::after, .el-table--group::after, .el-table::before {
  border: 0 !important;
  background: transparent !important;
}
</style>
