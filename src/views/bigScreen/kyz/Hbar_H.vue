  ...
  <div ref="HBar" class="h-bar" @mouseout="initInterval">
    ...
    <ul class="list">
      <li v-for="(item,index) in list" :key="index" @mouseenter="showTip(item,index)">
        ...
        <div class="title" :title="item.t">{{item.t}}</div>
        <div class="detail">
          <div v-if="item.p===0" class="ps" style="width:0px;">
            <i style="width:0px;"></i>
          </div>
          <div v-else class="ps" :style="{width:item.psw}">
            <i :style="{width:item.p,backgroundColor:bgFlag?colorList[index]:''}"></i>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      interVal: null,
      dataIndex: 4,
      colorList:['rgb(98,51,129)','rgb(63,151,161)'],
    }
  },
  // 属性
  props: {
  ...
  methods: {
    ...
    showTip(item, index, auto) {
      ...
      var realWidth = (this.$refs.HBar.offsetWidth - 20 -120- 10 - 20) * fullP;
      var realLeft = realWidth + 20 + 10+60;
      ...
      if (((index + 1) * 36 + this.$refs.HBarTip.offsetHeight) > this.$refs.HBar.offsetHeight) {
        this.tip.top = (index + 1) * 36 - this.$refs.HBarTip.offsetHeight - 28 + "px";
