	...
	<div ref="HBar" class="h-bar" @mouseout="initInterval">
		<div class="unit-leg-con">
			...
			<div v-if="legs.length>0" class="leg">
				<a><span class="leg-point short"></span>{{legs[0]}}</a>
				<a><span class="leg-point long"></span>{{legs[1]}}</a>
			</div>
		</div>
		<ul class="list">
		...
		<div ref="HBarTip" class="tip" :class="[tip.className]" :style="{top:tip.top,left:tip.left,display:tip.display}">
			<div style="font-weight: bold;">{{curItem.t}}</div>
			<div>总计：{{curItem.total}}</div>
			<div>实际：{{curItem.per}}（{{curItem.p}}）</div>
		</div>
	</div>
</template>
...
<style scoped="scoped">
	...
	.list>li:hover {
		background-color: rgba(101, 155, 207, .2);
	}
	...
	.tip {
		...
		min-height: 50px;
		padding: 5px;
		background-image: linear-gradient(to right, rgba(62, 130, 196, .9), rgba(62, 130, 196, .4));
		transition: all .3s;
	...
	.tip::before {
		...
		border-bottom: 8px solid rgba(62, 130, 196, .5);
