...
export default {
    ...
    components: {},
    methods: {
        drawPolygons:function(polygonData) {
            let that = this;
            savedAreas = [];
            polygonData.forEach(item => {
                ...
                //     html:item.name,
                // })
                savedAreas.push(polygon)
                polygon.on("click",(e) => {
                    that.$emit("click-area",e.target);
                })
        ...
        mapDoubleClick() {
            // L.polygon(points).addTo(this.map)
            ...
            //当框选区域完成以后，要通知上层组件已经框选完毕
            //并且把多边形的内容返回给上层
            //yinwaluo 20220125
            this.$emit("finish-area-select",PolygonPoints);
        },
        mapMove(e) {
            if (points.length > 0) {
