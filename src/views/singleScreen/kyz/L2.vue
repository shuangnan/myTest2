...
<template>
    <div ref="con" style="width: 100%;height:100%; padding-top: 8px;">
        <L21 ref="L2zd" :boxH="carouselHeight" v-show="fxjhSelectType===0" :rows="zdDataList" :showNum="showNum" @changeFXJHTab="changeFXJHTab" @openDialogC="openDialogC" data-type="zdfx"></L21>
        <L21 ref="L2zk" :boxH="carouselHeight" v-show="fxjhSelectType===1" :rows="zkDataList" :showNum="showNum" @changeFXJHTab="changeFXJHTab" @openDialogC="openDialogC" data-type="zk"></L21>
        <L21 ref="L2cj" :boxH="carouselHeight" v-show="fxjhSelectType===2" :rows="cjDataList" :showNum="showNum" @changeFXJHTab="changeFXJHTab" @openDialogC="openDialogC" data-type="cj"></L21>
        <L21 ref="L2sg" :boxH="carouselHeight" v-show="fxjhSelectType===3" :rows="sgDataList" :showNum="showNum" @changeFXJHTab="changeFXJHTab" @openDialogC="openDialogC" data-type="sg"></L21>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            var stime = nowDate.Format("yyyy-MM-dd hh:mm:ss");
        ...
        getDataCJ(stime, etime) {
            ...
            C.getData({
                ...
                r.forEach(t => {
                    ...
                    var typeSList = [];
                    typeList.forEach(v => {
                        typeSList.push(`<span style='color:${RISK_TYPECOLOR[v]}'>${RISK_TYPES[v]}</span>`)
                    });
        ...
        startScroll() {
            if (this.$refs.L2zk) {
                this.$refs.L2zk.endScroll();
            }
            if (this.$refs.L2cj) {
                this.$refs.L2cj.endScroll();
            }
            if (this.$refs.L2sg) {
                this.$refs.L2sg.endScroll();
            }
            if (this.$refs.L2zd) {
                this.$refs.L2zd.endScroll();
            }
            this.$refs.L2zd.startScroll();
            // if (this.zkDataList.length > 0) {
            //   this.$refs.L2zk.startScroll();
            // } else if (this.cjDataList.length > 0) {
            //   this.$emit("changeFXJHTab");
            // }
        },
        ft_time(date, ft_str) {
