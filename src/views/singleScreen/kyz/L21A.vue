    ...
    <div style="width:100%;height:100%;" @mouseenter="boxMouseEnter" @mouseleave="boxMouseLeave">
        ...
        >
            <el-carousel-item v-for="(item,index) in rows" :key="index">
                <div :id="dataType+'autoScroll_fx'+index" ref="autoScroll_fx" style="overflow:auto;width:100%;height:100%;">
                    <div
                    ...
                    >
                        <div style="width:75px;" class="flexCenter">{{temp.trackName}}</div>
                        <div
                            class="textOver"
                            style="width:65px;padding-left:3px;"
                            :title="temp.STN_NAME"
                        >
                            {{
                            temp.STN_NAME
                        ...
                        >
                            ...
                            <div v-if="dataType=='sg'">{{temp.arrTime}}</div>
                            <div
...
export default {
    ...
    methods: {
        ...
        scrollItem() {
            var dataList = this.rows[this.carouselActiveItem].children;
            if (dataList) {
                var temp = this.$refs.autoScroll_fx[this.carouselActiveItem];
                temp.scrollTop = 0;
                setTimeout(() => {
                    var dqTime = new Date().getTime();
                    var dqIndex = 0;
                    for (var i = 0; i < dataList.length; i++) {
                        var itemTime = new Date(dataList[i].seqTime).getTime();
                        if (itemTime > dqTime) {
                            dqIndex = i;
                            break;
                        }
                    }
                    var scrollTop = 60 * dqIndex - 60;
                    C.$("#"+this.dataType+"autoScroll_fx"+this.carouselActiveItem).animate({scrollTop:scrollTop+"px"},1000)
                    // var scollIH = scrollTop / 10;
                    // var setI = setInterval(() => {
                    //     temp.scrollTop += scollIH;
                    //     if (temp.scrollTop >= scrollTop) {
                    //         clearInterval(setI)
                    //     }
                    // }, 100);
                    // console.log("滚动数据", this.carouselActiveItem, this.rows[this.carouselActiveItem].children);
                }, 200);
