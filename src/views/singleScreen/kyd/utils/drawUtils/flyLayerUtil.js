export default {
    ...
    getFlyLine() {
        ...
        const showData = data.allLine.sort(function (a, b) {
            return b.num - a.num
        }).slice(0,100).map((line) => {
            const startCity = this.getGeoCoord(line.start);
            const endCity = this.getGeoCoord(line.end);

            //在循环中顺便组织下线动画数据
            if (Math.random() > 0.7) {
                this.curive(startCity, endCity, 50);
            }

            var temp = [startCity, endCity];
            return {
                count: line.num,
                geometry: {coordinates: temp, type: "LineString"},
            }
        });
    ...
    curive(fromPoint, endPoint, pointNum) {
        var delLng = (endPoint[0] - fromPoint[0]) / pointNum;
        var delLat = (endPoint[1] - fromPoint[1]) / pointNum;
	...
	getFlyData(data){
		this.timeData = [];
		const showData = data.sort(function (a, b) {
            return b.SL - a.SL
        }).slice(0,100).map((line) => {
		    const startCity = [Number(line.F_LON),Number(line.F_LAT)];
		    const endCity = [Number(line.D_LON),Number(line.D_LAT)];
		    //在循环中顺便组织下线动画数据
		    if (Math.random() > 0.2) {//随机选取线做动画
		        this.curive(startCity, endCity, 50);
		    }
		    var temp = [startCity, endCity];
		    return {
		        count: Number(line.SL),
		        geometry: {coordinates: temp, type: "LineString"},
		    }
		});
		return showData;
