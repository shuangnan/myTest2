export default {
    ...
    methods: {
		...
		openEdit(){
			...
			this.$store.dispatch('rect/toggleAllEdit', {flag: false})
			this.$parent.toggleEdit(!isEdit)
		},
		outprint(){
			var state = this.$store.state
			for (let i = 0, l = state.rect.rects.length; i < l; i++) {
			    console.log(JSON.stringify(state.rect.rects[i]))
			}
		},
		setVisible(){
			let t = this
			let visible = this.$parent.visible
			if(visible){
				t.visibleTxt = '一键显示'
			}else{
				t.visibleTxt = '一键隐藏'
			}
			this.$store.dispatch('rect/toggleVisible', {flag: visible})
			this.$parent.toggleVisible(!visible)
		}
    }
}