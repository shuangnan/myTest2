	...
	<el-container style="width: 100%;height: 100%;">
		<el-header style="height: 55px;color: white;">
			<div style="padding-top: 5px;">
				...
				<span style="float: right;"><el-checkbox v-model="filter.dandang">担当车</el-checkbox></span>
			</div>
			<div style="padding-top: 5px;">
...
<script>
	...
	export default {
		...
		methods: {
			...
			getData() {
				const _filter = {
					dandang:this.mapFilter.dandang,
					dongche: this.mapFilter.dongche,
					puche: this.mapFilter.puche,
					late_d1: this.mapFilter.late_d1,
					late_d2: this.mapFilter.late_d2,
					late_d3: this.mapFilter.late_d3,
					late_d4: this.mapFilter.late_d4,
					late_p1: this.mapFilter.late_p1,
					late_p2: this.mapFilter.late_p2,
					late_p3: this.mapFilter.late_p3,
					late_p4: this.mapFilter.late_p4,
				}
				this.filter = _filter;
				const tarinDC = [];
				const tarinPC = [];
				const lateD = this.trains.filter(v => {
					if (v.lateObj.lateCls !== "") {
						...
						v.starDay = v.START_DATE.substring(5, 16);
						v.endDay = v.END_DATE.substring(5, 16);
						if (v.trainType === "D") {
							tarinDC.push(v);
						} else {
							tarinPC.push(v);
