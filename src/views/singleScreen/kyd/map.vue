<template>
	<div @keydown.17="ctrlFlag=true" @keyup.17="ctrlFlag=false" style="width: 100%;height:100%;overflow: hidden;padding:0;margin:0;">
		<div style="position: absolute;top:80px;left:50%;margin-left:-100px;width: 200px;z-index: 410;">
			<el-autocomplete size="mini" placeholder="输入车次名定位..." clearable v-model="filterTrainStr" :fetch-suggestions="querySearch"
			 :trigger-on-focus="true" @select="filterTrain">
				<i slot="append" class="el-icon-search" @click="filterTrain('btn')"></i>
			</el-autocomplete>
		</div>
		<!-- 在途车统计 -->
		<div class="total-con" :class="{'min':total.min}" @click="total.min=!total.min">
			<i>统计</i>
			<div><span>晚点车：</span><label style="color: #dd2c0d;cursor: pointer;border-bottom: 2px solid #dd2c0d;" @click="toolClick('openLate')">{{filter.dandang?total.ddwd:total.wd}}</label></div>
			<div><span>担当车：</span><label style="color: #00aa00;">{{total.dd}}</label></div>
			<div><span>总列数：</span><label style="color: #55ffff;">{{total.all}}</label></div>
		</div>
		<div class="tools-con" :style="{height:!toolVisible?'30px':''}">
			<div class="tools-control" :class="{opened:toolVisible}" @click="toolVisible=!toolVisible">
				<i v-if="toolVisible" class="el-icon-d-arrow-right" style="font-size: 16px;cursor: pointer;transform: rotate(90deg);margin-top: -2px;"></i>
			...
			<div class="tools-item-con" :class="{show:toolVisible}">
				<div class="filter-group">
					...
					<div class="group-body">
						...
						<el-checkbox v-model="filter.dandang" label="担当车"><i class="filter-icon dandang"></i>担当车</el-checkbox>
					</div>
				</div>
				<div class="filter-group">
					<div class="group-title">桥隧</div>
					<div class="group-body">
						<el-checkbox v-model="filter.qiaoliang" label="桥梁"><i class="filter-icon qiaoliang"></i>桥梁</el-checkbox>
						<el-checkbox v-model="filter.suidao" label="隧道"><i class="filter-icon suidao"></i>隧道</el-checkbox>
					</div>
				</div>
				<div class="filter-group">
					<div class="group-title">天气</div>
					<div class="group-body">
						<el-checkbox v-model="filter.tianqi" label="天气"><i class="filter-icon tianqi"></i>天气情况</el-checkbox>
						<el-checkbox v-model="filter.tianqiyujing" label="天气预警"><i class="filter-icon tianqiyujing"></i>天气预警</el-checkbox>
					</div>
				</div>
				<div class="filter-group">
					<div class="group-title">区域<i v-if="filter_quyu.length>0" class="el-icon-edit" @click="quyuEdit=!quyuEdit" title="编辑区域"
						 style="margin-left: 10px;cursor: pointer;"></i></div>
					<div class="group-body">
						<template v-for="(qy,index) in filter_quyu">
							<span v-if="quyuEdit">
								<el-input v-model="qy.REGION_NAME" size="mini" style="width: 200px;"></el-input>
								<i class="el-icon-circle-check" style="margin: 0 5px;cursor: pointer;" title="保存" @click="handleUpdateQuyu(qy)"></i>
							...
							<el-checkbox v-else v-model="qy.visible" :label="qy.REGION_NAME" @change="handleFilterQuyu(qy)"></el-checkbox>
						</template>
					</div>
				</div>
				<div class="filter-group">
					...
					<div class="group-body" v-show="filter.dongche">
						<div @click="toolClick('openLate')" class="late-train-link">动车晚点</div>
						<el-checkbox v-model="filter.late_d1" :disabled="!filter.dongche" label="5-120分钟"><i class="filter-late wd1"></i>5-120分</el-checkbox>
						<el-checkbox v-model="filter.late_d2" :disabled="!filter.dongche" label="120-240分钟"><i class="filter-late wd2"></i>120-240分</el-checkbox>
						<el-checkbox v-model="filter.late_d3" :disabled="!filter.dongche" label="240-360分钟"><i class="filter-late wd3"></i>240-360分</el-checkbox>
						<el-checkbox v-model="filter.late_d4" :disabled="!filter.dongche" label="360分钟以上"><i class="filter-late wd4"></i>360分以上</el-checkbox>
					</div>
					<div class="group-body" v-show="filter.puche">
						<div @click="toolClick('openLate')" class="late-train-link">普车晚点</div>
						<el-checkbox v-model="filter.late_p1" :disabled="!filter.puche" label="5-240分钟"><i class="filter-late wd1"></i>5-240分</el-checkbox>
						<el-checkbox v-model="filter.late_p2" :disabled="!filter.puche" label="240-480分钟"><i class="filter-late wd2"></i>240-480分</el-checkbox>
						<el-checkbox v-model="filter.late_p3" :disabled="!filter.puche" label="480-720分钟"><i class="filter-late wd3"></i>480-720分</el-checkbox>
						<el-checkbox v-model="filter.late_p4" :disabled="!filter.puche" label="720分钟以上"><i class="filter-late wd4"></i>720分以上</el-checkbox>
					</div>
				</div>
				<div class="filter-group">
					<div class="group-title">操作</div>
					<div class="group-body flex">
						<el-button :type="drawFlag==='框选列车'?'primary':'info'" size="small" icon="el-icon-news" @click="startDraw">{{drawFlag}}</el-button>
						<el-button type="primary" size="small" @click="toolClick('openTask')" icon="el-icon-document">指派任务</el-button>
		...
		<!-- 地图 -->
		<div id="home-map" style="width: 100%;height: 100%;background: transparent;"></div>


		<!-- 列车详情 -->
		<el-dialog :close-on-click-modal="false" v-if="visible" :visible.sync="visible" top="3vh" title="车次信息" width="1200px"
		 :destroy-on-close="true" :append-to-body="false">
			<div style="height: calc(100vh - 100px);">
				<DetailInfo :datasource="detail" :detailSql="detailSql" @lczEvent="callPhone"></DetailInfo>
			</div>
		</el-dialog>
		<!-- 列车右键菜单 -->
		<TrainPhoneList v-if="visiblePhone" :style="phoneListStyle" :cc="phoneListCC" @callPer="callPhone"></TrainPhoneList>
		<!-- 拨打电话组件 -->
		<call ref="call" :visibleController="false" :targetNumber="targetNumber" :initGroupNo="initGroupNo" :initPosition="ipos"
		 style="z-index:9000"></call>
		<!-- 正晚点信息 -->
		<el-dialog :close-on-click-modal="false" :visible.sync="visibleLate" title="正晚点信息" width="800px" :destroy-on-close="true"
		 :append-to-body="true">
			<div style="height: 600px">
				<MapTrainLate v-if="visibleLate" :trains="dataTrain" :mapFilter="filter"></MapTrainLate>
			</div>
		</el-dialog>
		<!-- 旅服信息 -->
		<el-dialog :close-on-click-modal="false" :visible.sync="visibleTravel" title="旅服信息" width="800px" :destroy-on-close="true"
		 :append-to-body="true">
			<div style="height: 600px">
		...
		<!-- 区域编辑窗口 -->
		<el-dialog :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false" :visible.sync="visibleQuYu"
		 title="区域编辑" width="500px" style="margin-top: 100px;" :destroy-on-close="true" :append-to-body="true">
...
<script>
	...
	import DetailInfo from "./way/detail.vue" //列车详情
	...
	export default {
		...
		data() {
			return {
				...
				markerDiff: ["", 6, 2.5, 1.6, 1.1, 0.9, 0.55, 0.42, 0.3, 0.22, 0.11, 0.055], //桥隧、天气、预警、车信息marker向下偏移量
				total: {
					min: false,
					wd: "-",
					all: "-",
		...
		watch: {
			...
			ccname(value) {
				if (value) {
					let targetTrain = this.trainData.find(train => train.TRAIN === value);
					if (targetTrain) {
			...
			"filterTrainStr"(v) {
				this.filterTrainStr = this.Tools.parseTrainCode(v);
			}
		},
		mounted() {
			// 绑定 查看列车统计图方法
		...
		methods: {
			...
			getDatas() {
				...
				this.getData_qiaoSui(() => {
					this.getData_onRoadTrain(true); //查询在途列车
				});
				setInterval(() => {
			...
			querySearch(query, cb) {
				let d = [];
				if (query) {
					d = this.dataTrain.filter(v => v.TRAIN.indexOf(query.toUpperCase()) > -1);
				}
				cb(d);
			},
			handleSelect() {
			...
			// 全局搜索车次定位
			filterTrain(p) {
				const d = p === "btn" ? this.dataTrain.find(v => v.TRAIN.indexOf(this.filterTrainStr.toUpperCase()) > -1) : p;
				if (d) {
					C.$(".filtered").removeClass("filtered");
					const toZoom = this.mapZoom < 10 ? 10 : this.mapZoom;
			...
			callPhone(per, ty, isMore) {
				if (ty === "phone") {
					...
					const pos = "&unitFlag=1&UserName=" + webP.UserName + "&UserPID=" + webP.UserPID + "&Oid=" + webP.Oid + "&Oname=" +
						webP.Oname;
					const rec = "&userNameList=" + userNameList.join(",") + "&userPidList=" + userPidList.join(",");
					const url = C.ADDS.url_34_79 + "/JFLCMS/vue/#/XYLink?pageType=1" + pos + rec; //
					// const url = "https://10.194.72.31:8081/#/XYLink?pageType=1" + pos + rec;
					setTimeout(() => {
						this.$emit("openXiaoYu", url);
						//window.open(url);
						this.loading.close();
					}, 3000);
				}
			...
			haddleCloseQuyu() {
				...
				this.startDraw();
				// mapUtil.clearEditLines();
				this.formQuYu.layer.removeFrom(this.map);
			},
			haddleSaveQuyu() {
				this.endDrawLines((kxTrains) => {
					...
					home.saveRegion({
						s: [f.REGION_ID, f.REGION_NAME, f.REGION_TYPE, f.WARN_MESSAGE, kxTrains, points, this.$store.state.params.UserPID],
					}).then(r => {
						if (r) {
							this.$message({
								message: '添加成功',
			...
			getSelectTrains() {
				...
				C.$(".kuang-xuang").each(function() {
					...
					if ($vm.filter.dandang) {
						if (C.$(this).hasClass("ddc-1")) {
							trainids.push(id);
						}
					} else {
						trainids.push(id);
				...
				const cks = this.dataTrain.filter(train => trainids.indexOf(train.TRAIN_ID) > -1);
				return cks;
			},
			// 绑定marker点击事件---桥遂
			setMarkerClick_qiaoSui(marker, data) {
				marker.on("click", (e) => {
					...
					this.map.setView([parseFloat(e.latlng.lat) + this.markerDiff[this.mapZoom], e.latlng.lng]);
					L.marker(e.latlng, {
						icon: nameDivIcon
					}).addTo(this.map);
					setTimeout(() => {
			...
			setTrainMarkerClick(param) {
				...
				if (count !== "") {
					const isSingle = C.$("#" + train.TRAIN_ID).parent().hasClass("single-train");
					C.$("#" + train.TRAIN_ID).parent().addClass("single-train");
					let trains = this.dataPointTrain[train.LAT + "|" + train.LON] || [];
					if (trains.length > 0 && isSingle === false) {
						C.$.each(trains, (i, d) => {
							C.$("#" + d.TRAIN_ID).parent().addClass("single-train");
							if (i > 0) {
								let mtop = -6 + "px";
								if (this.mapZoom >= 5) {
									mtop = -i * 26 - 6 + "px";
				...
				const trainInfoPath = prev + "?UserName=" + UserName + "&UserPID=" + UserPID + "&Oid=" + Oid + "&Oname=" +
					Oname + '&id=' + train.TRAIN_ID + '&cc=' + train.TRAIN;
				let nameDivIcon = L.divIcon({
					className: 'textFonts',
					iconSize: [10, 10],
					popupAnchor: [0, 0],
					html: `<div class="leaflet-popup leaflet-zoom-animated" style="opacity: 1; transform: translate3d(0px, -25px, 0px); bottom: 0px; left:-340px;">
							<div class="leaflet-popup-tools" style="margin-right:5px;">
								<a onclick="window.showTrainCharts('${train.TRAIN_ID}')">作业信息</a>
							</div>
							<a class ="leaflet-popup-close-button" style="z-index:10;" href="javaScript:void(0)" onclick="$('.leaflet-popup').remove();">x</a>
							<div class = "leaflet-popup-content-wrapper blue">
				...
				this.map.setView([parseFloat(train.LAT) + this.markerDiff[this.mapZoom], train.LON]);
			...
			onMapZoom(e) {
				...
				if (oldRoom !== newZoom) {
					if (newZoom >= this.initZoom && oldRoom < this.initZoom) { //放大
						C.$(".marker-tarin").removeClass("min");
						this.setTrainCount();
					} else if (newZoom < this.initZoom && oldRoom >= this.initZoom) { //缩小
			...
			filterMarkers(a, b) {
				...
				C.$(".marker-tarin").parent().hide(); //全部隐藏
				const wdCls = [];
				if (this.filter.late_d1 && this.filter.dongche || this.filter.late_p1 && this.filter.puche) {
					wdCls.push("wd1");
				}
				if (this.filter.late_d2 && this.filter.dongche || this.filter.late_p2 && this.filter.puche) {
					wdCls.push("wd2");
				}
				if (this.filter.late_d3 && this.filter.dongche || this.filter.late_p3 && this.filter.puche) {
					wdCls.push("wd3");
				}
				if (this.filter.late_d4 && this.filter.dongche || this.filter.late_p4 && this.filter.puche) {
					wdCls.push("wd4");
				}
				// 动、普都显示,且有晚点条件
				if (this.filter.puche && this.filter.dongche && wdCls.length > 0) {
					// 动车未晚点，则将动车显示
					if (!this.filter.late_d1 && !this.filter.late_d2 && !this.filter.late_d3 && !this.filter.late_d4) {
						wdCls.push("type-D");
					}
					// 普车未晚点，则将普车显示
					else if (!this.filter.late_p1 && !this.filter.late_p2 && !this.filter.late_p3 && !this.filter.late_p4) {
						wdCls.push("type-P");
					}
				}
			...
			creatMarks_curWeather(source) {
				...
				C.$.each(source, (index, item) => {
					const imgUrl = C.ADDS.weatherUrl + item.ICON + "-fill.svg";
			...
			creatMarker_yujingWeather(source) {
				...
				C.$.each(source, (index, item) => {
					// BRI_HOLE_NAME 名称
					const imgUrl = C.ADDS.weatherUrl + item.level + "预警.png";
					const marker = L.marker([item.LAT, item.LON], {
						icon: L.divIcon({
							className: 'textFonts',
							iconSize: [30, 30],
						...
						this.map.setView([parseFloat(item.LAT) + this.markerDiff[this.mapZoom], item.LON]);
			...
			getFutureWeather(e) {
				if (this.ctrlFlag) {
					...
					const trainInfoPath = prev + "?lat=" + p.lat + "&lng=" + p.lng + "&StnCode=" + (e.StnCode ? e.StnCode : "");
					let divIcon = L.divIcon({
						className: 'textFonts',
						iconSize: [10, 10],
						popupAnchor: [0, 0],
					...
					this.map.setView([parseFloat(p.lat) + this.markerDiff[this.mapZoom], p.lng]);
			...
			trainMessageArr(msgTrain) {
				...
				// 5层上 图标车
				const existTrains = this.dataPointTrain[msgTrain.LAT + "|" + msgTrain.LAT];
				if (el) {
					...
					// 地图中没有此车
					const tempTrain = this.dataTrain.find(item => item.TRAIN_ID === msgTrain.TRAIN_ID);
					if (!tempTrain) {
						...
						if (typeof(existTrains) === "undefined") {
							...
							this.dataPointTrain[msgTrain.LAT + "|" + msgTrain.LON].push(msgTrain);
						}
					}
				}
			},
			...
			getClxq(item) {
				...
				this.visible = true
				let t = this
				const _date = echarts.format.formatTime('yyyyMMdd', new Date(item.START_DATE).getTime());
			...
			handleUpdateQuyu(f) {
				// //sql [区域id,"区域名称",区域类型（0-不发送警戒消息，1-发送警戒消息）,"警戒消息内容","列车列表","点列表","客运段"]	
				home.saveRegion({
					s: [f.REGION_ID, f.REGION_NAME, f.REGION_TYPE, f.WARN_MESSAGE, "", "", this.$store.state.params.UserPID],
				}).then(r => {
					if (r) {
						this.$message({
							message: '修改成功',
...
<style scoped="scoped">
	.tools-con {
		...
		background-image: linear-gradient(-3deg, #0e1331 0%, #0d1567 100%), linear-gradient(#0e1743, #0e1743);
...
<style>
	...
	.total-con {
		...
		height: 160px;
		padding: 20px;
		background-image: linear-gradient(#0d2067 0%, #08122d 100%);
		box-shadow: 0px 0px 30px 8px rgba(3, 12, 36, 0.57);
		z-index: 410;
		white-space: nowrap;
		overflow: hidden;
		transition: all .3s;
