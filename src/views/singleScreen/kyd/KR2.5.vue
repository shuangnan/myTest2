...
<template>
    <div class="main" style="width:100%;height:100%;padding-right:15px;padding-bottom:0px;">
        <div style="height:40px;width:100%;" class="KRTitle">库存情况</div>
        <div style="height:30px;width:100%;display:flex;justify-content:flex-end;">
            <div v-for="(item,index) in colorList" :key="index" class="KR25Tool">
                <div :style="{background:item.color}"></div>
                <span>{{item.text}}</span>
            </div>
        </div>
        <div ref="chartKR25" style="width:100%;height:calc(100% - 70px)"></div>
    </div>
</template>
...
export default {
    data() {
        return {
            ...
            loop: null,
            colorList:[
                {text:"库存不足",color:"#844008"},
                {text:"合理库存",color:"#2b6a09"},
                {text:"库存超量",color:"#948208"},
            ]
        }
    },
    props: {
    ...
    methods: {
        ...
        initCharts() {
            ...
            this.myChart = this.$echarts.init(this.$refs.chartKR25);
            this.myChart.resize();
            ...
            let option = {
                ...
                tooltip: {
                    ...
                    formatter: function (params) {
                        var items = [];
                        params.forEach(v => {
                            items.push(`<div>${v.marker} ${v.seriesName}：${v.value}</div>`);
                        })
