	...
	<div class="container">
		...
		>
			...
			>
                ...
                :style="{
                    color: tellColor(person.O_NAME)
                }"
								>{{ person.DUTY_MAN }}</span
							>
						</div>
						<div
							style="width: 30%; display: flex; align-content: center; align-items: center; justify-content: center"
						>
							<span
								style="font-weight: bold; font-size: 45px"
                :style="{
...
<script>
	...
	export default {
      ...
      tellColor(oname, isCopy = false) {
        let colorIndex = this.allUnits.findIndex((o) => {
          return o === oname.split('（')[0]
        })
        if (this.allUnits.length % 2 === 0) {
          // 单位个数偶数个, copyHtml使用相同颜色序列
          return colorIndex % 2 === 0 ? 'white' : this.anotherColor
        } else {
          return isCopy ? (colorIndex % 2 === 0 ? this.anotherColor : 'white') : (colorIndex % 2 === 0 ? 'white' : this.anotherColor)
          // 单位个数奇数个, copyHtml使用相反颜色序列
        }
        // console.log(oname, colorIndex)
        // return Math.abs(this.allUnits.length % 2 === 0 ? colorIndex : (colorIndex + this.circulateTimes)) % 2 === 0 ? 'white' : this.anotherColor
      },
			getAllPersons() {
				C.getDataP123(
					{
					...
					r => {
						this.allPersons = r["1"].filter((p) => {
              return p.DUTY_MAN !== ''
            });
            let unitsDic = r['1'].map((o) => {
              return o.O_NAME.split('（')[0]
            })
            this.allUnits = Array.from(new Set(unitsDic))
            console.log('ALL UNITS', this.allUnits)
            console.log('ALL PERSON', this.allPersons)
						...
						this.$nextTick(() => {
							...
							// 	"subCommandContainer"
							// ).innerHTML;
              let cloneObj = document.getElementById("subCommandContainer").cloneNode(true)
              cloneObj.childNodes.forEach((n) => {
                // console.log(n)
                // console.log(n.childNodes[1].childNodes[0].innerHTML)
                let oname = n.childNodes[1].childNodes[0].innerHTML
                let color = this.tellColor(oname, true)
                n.childNodes.forEach((d) => {
                  d.childNodes[0].style.color = color
                })
              })
              this.copyHtml = cloneObj.innerHTML
      ...
      scroll() {
        ...
        if (this.scrollTop >= scrollItemBox) {
          this.circulateTimes = this.circulateTimes + 1
          this.scrollTop = 0
          this.$nextTick(() => {
            this.copyHtml = document.getElementById(
              "subCommandContainer"
